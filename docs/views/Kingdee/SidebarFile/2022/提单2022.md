...
<span style="font-size:22px;">共计283个</span>
...
- 第28周(7.4-7.10) 13
    ...
    **解决方案**：二开直接sql账表，通过如下sql取数
    ...
    ```sql
    select t1.FBILLNO as JDSCRKDNO,T2.FMATERIALID,T5.FNUMBER,T2.FBaseRealQty AS JCDWSHSL,t4.FBILLNO,(t3.FBaseOutQty * -1) as JBDWSTSL from T_SP_INSTOCK t1 
    ...
    cinner join T_SP_OUTSTOCK t4 on t3.FID=t4.FID
    ```
